...
# Carga de paquetes
pacman::p_load(ggrepel, plotly, purrr, readxl, rio, scales, tidyverse)

# Gráfico 1. Cantidad de académicos por género en la USACH (2018-2024)----
# Cargar datos
ema <- read_excel("01-input/data-orig/CONSOLIDADO EMA 2018-2024_USACH.xlsx")
...
plot_acad_genero <- plot_ly(
  ...
  layout(
    title = list(
      text = "<b>Académicos por género</b>",
      font = list(size = 14),
      x = 0.5
    ),
...
# Cargar datos
acad_grados <- read_excel("01-input/data-orig/CONSOLIDADO EMA 2018-2024_USACH.xlsx")
...
plot_acad_grado <- ggplotly(p_facet, tooltip = "text") %>%
  layout(
    title = list(
      text = "<b>Académicos por género y grado académico</b>",
      font = list(size = 14),
      x = 0.5),
    margin = list(t = 80))
...
academicos_jerarquia <- ema_jq %>%
  ...
  mutate(
    total_jerarquia = sum(valor[tipo == "detalle"], na.rm = TRUE),
  ...
  mutate(
    grupo = interaction(jerarquia, genero, sep = " ", lex.order = TRUE),
...
plot_jerarquias <- academicos_jerarquia%>%
  ...
  layout(
    title = list(
      text = "<b>Académicos por género y jerarquía (1)</b>",
      font = list(size = 14),
      x = 0.5
    ),
...
plot_jerarquias_2 <- academicos_jerarquia_2 %>%
  ...
  layout(
    title = list(
      text = "<b>Académicos por género y jerarquía (2)</b>",
      font = list(size = 14),
      x = 0.5
    ),
    xaxis = list(title = "", tickvals = 2018:2024),
...
acad_pub <- ema_pub %>%
  ...
  mutate(
    total = n_pub_f + n_pub_m,
    prop_f = round(n_pub_f / total * 100),
    prop_m = round(n_pub_m / total * 100)
  ) %>%
  pivot_longer(
...
plot_pub <- acad_pub %>%
  ...
  layout(
    title = list(
      text = "<b>Publicaciones en revistas indexadas por género</b>",
      font = list(size = 14),
      x = 0.5
    ),
    xaxis = list(title = "", tickvals = 2018:2024),
...
# Combinar subplots
plot_jd_ca <- subplot(subplots_1, nrows = 2, shareX = TRUE, shareY = TRUE, titleY = TRUE) %>%
  layout(
    title = list(
...
# Combinar subplots
plot_d_vd <- subplot(subplots_2, nrows = 2, shareX = TRUE, shareY = TRUE, titleY = TRUE) %>%
  layout(title = list(
...
plot_tr <- ema_tr %>%
  ...
  layout(title = list(
    text = "<b>Tasa de retención por género</b>",
    font = list(size = 14),
    x = 0.5),
    xaxis = list(title = "", tickvals = 2018:2024),
    yaxis = list(
...
plot_tr_area <- subplot(plots_area, nrows = n_rows, shareX = TRUE, shareY = TRUE, margin = 0.04) %>%
  layout(title = list(
